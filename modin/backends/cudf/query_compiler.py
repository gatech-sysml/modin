...
def _str_map(func_name):
    """
    Finds a function from cudf.core.column.string.StringMethods and creates and returns
    a function that runs that function for some given dataframe and parameters
...
def _dt_prop_map(property_name):
    """
    Creates a function that finds a dt attribute of a given dataframe and returns it as a dataframe.
...
def _dt_func_map(func_name):
    """
    Finds a function from cudf.Series.dt and creates and returns a function that runs that
    function for some given dataframe and parameters
...
class cuDFQueryCompiler(PandasQueryCompiler):
    ...
    def transpose(self, *args, **kwargs):
        """
        Transposes the dataframe by turning rows into columns and vice-versa

        Parameters
        ----------
        args
            A dictionary unused in this method
        kwargs
            A dictionary unused in this method
            
        Returns
        -------
            A new updated cuDFQueryCompiler with the rows and columns switched
    ...
    """
        A workaround for multi-element assignment with cudf

        Parameters
        ----------
        row_numeric_index
            the rows over which the function is applied
        col_numeric_index
            the columns over which the function is applies
        broadcasted_items
            values to which items in the frame are assigned
            
        Returns
        -------
            A new updated cuDFQueryCompiler with the indicated values changed
    ...
    def drop(self, index=None, columns=None):
        """
        Removes the given indicated indices and columns from the dataframe

        Parameters
        ----------
        index
            An array containing the indexes/rows to be left out of the frame
        columns
            An array containing the columns to be left out of the frame
            
        Returns
        -------
            A new updated cuDFQueryCompiler with the indicated rows and columns not included
    ...
    def default_to_pandas(self, pandas_op, *args, **kwargs):
        """
        Runs the given operation through Pandas

        Parameters
        ----------
        pandas_op
            The pandas operation to be run
        args
        ...
        Returns
        -------
            The result of the `pandas_op`, converted back to CuDFQueryCompiler
    ...
    def where(self, cond, other, **kwargs):
        """
        Uses cudf's where method to replace values which are False wrt the cond with values from other

        Parameters
        ----------
        cond
            A condition that determines which values to replace
        other
            The other dataframe with the values used to replace the values in this frame
        kwargs
            A dictionary containing parameters for the where function
        
        Returns
        -------
            A new updated cuDFQueryCompiler with the appropriate values replaced
    ...
    def reindex(self, axis, labels, **kwargs):        
        """
        Fits a new index for this dataframe/series

        Parameters
        ----------
        axis
            The axis index object to target the reindex on
        labels
            New labels to conform 'axis' on to
        kwargs
            A dictionary containing parameters for the reindex function
            
        Returns
        -------
            A new QueryCompiler with the index updated
    ...
    def mean(self, **kwargs):
        """
        Calculates the means for the dataframe

        Parameters
        ----------
        kwargs
            A dictionary that is currently unused

        Returns
        -------
            A new updated cuDFQueryCompiler that contains the calculated means of the columns
    ...
    def to_datetime(self, **kwargs):
        """
        Creates through MapFunction.register and uses on self a function that sets values to datetime type

        Parameters
        ----------
        kwargs
            A dictionary that contains parameters for this function

        Returns
        -------
            A new updated cuDFQueryCompiler with values converted to datetime format
    ...
    def dt_total_seconds(self):
        """
        A method that uses pandas to convert time to seconds
        
        Returns
        -------
            A new cuDFQueryCompiler with times converted to seconds
    ...
    def isin(self, values):
        """
        Returns a dataframe which tells whether each item in this item's frame is in the given list

        Parameters
        ----------
        values
            A list, or a pandas.Index object that is converted to a list, for which you're checking whether the values in the frame are in

        Returns
        -------
            A new updated CuDFQueryCompiler with booleans that tell which items were in the values
    ...
    def unique(self, **kwargs):
        """
        Returns the unique entries in the data

        Parameters
        ----------
        kwargs
            A dictionary that is unused in this method

        Returns
        -------
            A new updated cuDFQueryCompiler with no duplicates
    ...
    def nunique(self, **kwargs):
        """
        Returns the number of unique entries in each row or column
        ...
        Returns
        -------
            A new updated cuDFQueryCompiler with the number of unique entries in either each row or column
    ...
    def std(self, **kwargs):
        """
        Does the standard deviation operation on the dataframe
        ...
        Returns
        -------
            A new updated cuDFQueryCompiler whose frame has the std of the columns of the original frame
    ...
    def var(self, **kwargs):
        """
        Does the variation operation on the dataframe
        ...
        Returns
        -------
            A new updated cuDFQueryCompiler whose frame has the variation of the columns of the original frame
    ...
    def fillna(self, **kwargs):
        """
        Fills in the na values in the frame

        Parameters
        ----------
        kwargs
            A dictionary with the parameters used in the fillna function

        Returns
        -------
            A new updated cuDFQueryCompiler with the na values replaced
    ...
    def view(self, index=None, columns=None):
        """
        Returns a QueryCompiler that only has the given rows and columns

        Parameters
        ----------
        index
            The indices of the rows to be included in the new QueryCompiler
        columns
            The indices of the columns to be included in the new QueryCompiler
        Returns
        -------
            A new QueryCompiler with the indicated rows and columns
    ...
    def sort_rows_by_column_values(self, by, ascending=True, **kwargs):
        """
        Sorts the rows of the frame based on the values of a column

        Parameters
        ----------
        by
            Used to set "by" parameter for the sort_values method
        ascending
            Whether to sort by ascending or descending order
        kwargs
            A dictionary that contains the parameters used in the sort_values function
            
        Returns
        -------
            A new updated cuDFQueryCompiler with sorted rows
    ...
    def sort_columns_by_row_values(self, by, ascending=True, **kwargs):
        """
        Sorts the columns of the frame based on the values of a row
        ...
        Returns
        -------
            A new updated cuDFQueryCompiler with sorted columns
    ...
    def reset_index(self, **kwargs):
        """
        Performs the reset_index operation on the dataframe

        Parameters
        ----------
        kwargs
            A dictionary containing parameters used in the reset_index function

        Returns
        -------
            A new updated cuDFQueryCompiler
    ...
    def apply(self, func, axis, *args, **kwargs):
        """
        Tries to apply the function using the methods below this one
        ...
        axis
            Whether the function is to be applied row-wise or column-wise
        args
            A dictionary containing parameters to be used with the function
        kwargs
            A dictionary containing parameters to be used with the function
    ...
    def _apply_text_func_elementwise(self, func, axis, *args, **kwargs):
        ...
        ----------
        func
            A string used to indicate the function to be applied
        axis
            Whether the function is to be applied row-wise or column-wise
        args
            A dictionary that is unused in this function
        kwargs
            A dictionary that is unused in this function
    ...
    def _callable_func(self, func, axis, *args, **kwargs):
        ...
        ----------
        func
            The function to be applied, which is callable
        axis
            Whether the function is to be applied row-wise or column-wise
        args
            A dictionary that is unused in this function
    ...
    def _dict_func(self, func, axis, *args, **kwargs):
        """
        Applies a dictionary to the frame 

        Parameters
        ----------
        func
            The dictionary to be applied
        axis
            Whether the function is to be applied row-wise or column-wise
        args
            A dictionary containing parameters to be used with the function
    ...
    def _list_like_func(self, func, axis, *args, **kwargs):
        """
        Applies a list-like object to the frame

        Parameters
        ----------
        func
            The list-like object to be applied
        axis
            Whether the function is to be applied row-wise or column-wise
        args
            A dictionary containing parameters to be used with the function
    ...
    def applymap(self, func, out_dtype=None):
        """
        Applies the function to each element in the dataframe
        ...
        func
            The function to be applied
        out_dtype
            I would guess the datatype of the value in the returned frame
        Returns
        -------
            A new updated CuDFQueryCompiler with the function implied
    ...
    def duplicated(self):
        """
        Returns which rows are and aren't duplicated

        Returns
        -------
            A new updated cuDFQueryCompiler with booleans regarding which rows are duplicates of others
    ...
    def hash_values(self):
        """
        Hashes the columns of the dataframe


        Returns
        -------
            A new updated cuDFQueryCompiler with the hashes of the values of the data
    ...
    def value_counts(self, **kwargs):
        ...
        ----------
        kwargs
            A dictionary containing the parameters used for this method

        Returns
        -------
        A new updated CuDFQueryCompiler with counts of the unique values
        """
    ...
    def _resample_func(
        ...
        """
        Reorganizes the data to group by some date or time

        Parameters
        ----------
        resample_args
            The arguments to be passed into the resample methid
        func_name
            The function to be used from pandas.core.resample.Resampler
        new_columns
            unused
        df_op
            unused
        args
            A dictionary passed to the resample function
        kwargs
            A dictionary passed to the resample function
        
        Returns
        -------
            A new updated QueryCompiler grouped by date/time
    ...
    def diff(self, *args, **kwargs):
        """
        Finds the difference between dataframe elements (i.e. the differences between each row)

        Parameters
        ----------
        args
            A dictionary passed to the diff function
        kwargs
            A dictionary passed to the diff function

        Returns
        -------
            A new updated cuDFQueryCompiler where each value is a difference between values in the old one
